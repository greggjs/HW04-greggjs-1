...
int greggjsStarbucks::importData(Entry* import, string filename) {
    try{
		...
		while(!fid.eof()){
			...
			fid.getline(buffer,256,','); /// get the identifier
			temp->identifier = (string)buffer;
            
			fid.getline(buffer,256,','); /// get the x coordinate
			temp->x  = atof(buffer); 
            
			fid.getline(buffer,256,'\r'); /// get the y coordinate
			temp->y  = atof(buffer);
...
void greggjsStarbucks::build(Entry *c, int n) {
    ...
    for (int i = 1; i < n; i++) {
        ...
        new_node->data_ = &c[i];
        new_node->color_ = Color8u((int)(new_node->data_->x*255),(int)((1-new_node->data_->y)*255),(int)(new_node->data_->x*new_node->data_->y*255));
...
void greggjsStarbucks::getArea(Vec2f top_left, Vec2f bottom_right, Node* root, bool xLevel, LinkList* list) {
    ...
    if (((xLevel) ? top_left.x : top_left.y) < ((xLevel) ? root->data_->x : root->data_->y)) {
    ...
    if (temp->x >= top_left.x && temp->x<= bottom_right.x && temp->y >= top_left.y && temp->y <= bottom_right.y) {
        ...
        list_temp->vec_ = Vec2f(root->data_->x, root->data_->y);
